...
func NewHandler(submitter Submitter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, "Geocoder request failed.", http.StatusBadRequest)
			return
		}

		result, err := submitter.Submit(request)
		if err != nil {
			http.Error(w, "Geocoder request failed.", http.StatusInternalServerError)
			return
		}
...
func writeResponse(w http.ResponseWriter, result *Result) {
	if body, err := json.Marshal(result); err == nil {
		...
		http.Error(w, "Failed to marshal Geocoder result", http.StatusInternalServerError)
	}
}
